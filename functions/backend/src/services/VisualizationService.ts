    ...
    try {
        ...
        if (prevGoalProgress.length === updatedGoalProgress.length) {
            for (const [updatedDateKey, updatedValue] of updatedGoalProgress) {
                ...
                const dateKey = [GoalType.DAILY, GoalType.ONE_TIME].includes(updatedGoal.goalType) ? updatedDateKey : format(new Date(), APP_CONSTANTS.DATE_FORMAT);
                const visualizationSnapshot = await VisualizationRepository.getVisualizationSnapshot(dateKey, userId);
                ...
                if (!updatedValue) {
                    ...
                    if (!visualizationSnapshot.empty) {
                        ...
                        await VisualizationRepository.updateVisualization({
                            ...data,
                            tasksCompleted: Math.max(data.tasksCompleted - 1, 0)
                        });
                    }
                } else {
                    // user checked
